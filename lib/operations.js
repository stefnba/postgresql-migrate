...
exports.newMigrationFile = exports.runMigrations = exports.listAppliedMigrations = exports.readMigrationFile = exports.readMigrationsFiles = void 0;
...
const MIGRRATION_DIR = path_1.default.join(path_1.default.dirname(index_1.CONFIG_FILE), index_1.CONFIG.dir);
/**
 * Reads all files and selects .sql ones in migration dir
 */
const readMigrationsFiles = () => {
    const dirFiles = (0, fs_1.readdirSync)(MIGRRATION_DIR);
    ...
    dirFiles.forEach((f) => {
        if (f.endsWith('.sql')) {
            ...
            migrationFiles.push({
                fullpath: path_1.default.join(MIGRRATION_DIR, f),
                name: f,
                ts,
                title
            });
 ...
 * @param name
 */
const newMigrationFile = (name) => {
    // todo replace chars in name string
    const template = (0, fs_1.readFileSync)(index_1.DEFAULTS.templateFile, { encoding: 'utf-8' });
    const filename = `${(0, dayjs_1.default)().valueOf()}_${name}.sql`;
    // const filename = `${dayjs().format('YYYYMMDD-HHmmssSSS')}_${name}.sql`;
    const fullpath = path_1.default.join(MIGRRATION_DIR, filename);
